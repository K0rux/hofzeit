...
type EditCostCenterDialogProps = {
...
export function EditCostCenterDialog({
  ...
  // Update form when cost center changes
  useEffect(() => {
    setFormData({
      name: costCenter.name,
      number: costCenter.number || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Fehler beim Aktualisieren der Kostenstelle')
      }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Bearbeite die Details der Kostenstelle
          </DialogDescription>
        </DialogHeader>
